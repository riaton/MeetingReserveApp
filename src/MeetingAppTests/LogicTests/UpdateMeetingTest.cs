...
public class UpdateMeetingTest
{
    ...
    public async Task 正常終了時のテスト()
    {
        ...
        _mock.Setup(x => x.Update(It.IsAny<UpdateMeetingRequestModel>()))
            .ReturnsAsync(CommonResult.OK);

        var res = await _func.UpdateMeetingHandler(_req, _context);
    ...
    public async Task バリデーションのテスト()
    {
        ...
        _model.Title = null;
        _req.Body = JsonSerializer.Serialize(_model);
        res = await _func.UpdateMeetingHandler(_req, _context);
        Assert.Equal(CommonResult.ValidateError, res.StatusCode);
        //必須パラメータ(Room)なし
        _model.Title = "aaa";
        _model.Room = null;
